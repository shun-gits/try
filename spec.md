...
- v0.1 → v0.2:
  ...
  - 未決事項を [TBD] マーカーで明示
- v0.2 → v0.3（実装フェーズで確定した事項を反映）:
  - 必須ローテーション（乗客単位・B/D 交互順）を新節 §17 として明文化（D 需要の駆動力）
  - 滞在 max（帰還期限）は期限が計画期間内のときのみ帰還を強制する旨を §13 に明記
...
initial_state:
  passengers:
    ...
    - passenger_id: P002
      location: A
  # 注: 常駐 min を持つ各 B 島は、開始時点で常駐者を置かないと t=0 で違反する。
  #     各 B 島の初期常駐者はインスタンスパラメータとして必ず与える（§22）。
...
帰還期限と計画末（確定）:
  max_hours（帰還期限）は、期限が計画期間内に収まる場合のみ帰還を強制する。
  すなわち arrival + max_hours ≤ horizon_end のとき帰還必須。
  期限が計画末を超える「最後の常駐者」は、計画末まで滞在してよい（帰還イベントを持たない）。
  これが無いと計画末で無限に交代が必要となり実行不能化するため。
...
type: together
  指定カテゴリ群は、入域便(A_Bx)・退域便(Bx_A)のいずれにおいても全員存在または
  全員不在のいずれか（入域・退域に対称適用）。すなわち群はペアで島に入り、ペアで島を出る。
  例（B1）: Category1 だけ乗車 → NG / Category2 だけ → NG / 両方 → OK / 両方なし → OK
  （同一便に対する判定。入域便・退域便それぞれで成立すること。）

同乗者の滞在（確定）:
  together で A_Bx 便に同乗した者は、その島の「滞在者」になる（便だけ同乗して即戻ることはしない）。
  常駐要件（occupancy / category_requirements）を満たすためのカテゴリでなくても、
  到着した以上は §13 の stay（min/max）と必須ローテーション（B の次は D）の対象になる。
  例: B1 が常駐に必要とするのは Category1 のみだが、together 相棒の Category2 も
      B1 に滞在し、後で帰還・D 勤務へローテーションする。
  ※この帰結として、相棒カテゴリの供給が不足するとローテーションが連鎖して
    実行不能になり得る（インスタンス設計上の制約）。
...
17. Rotation Constraints（必須ローテーション）

D への輸送需要を生む駆動力。乗客単位の「勤務交互順」制約。

- 各乗客の連続する勤務（滞在）は、B 勤務と D 勤務を交互に行わなければならない。
  すなわち B 滞在の次の滞在は D、D 滞在の次は B でなければならない。
  同種の連続（B→B / D→D）は不可。
- 適用単位は「乗客」（カテゴリ単位ではない）。
- 帰結: 常駐要員プールが限られる下では、同じ乗客が再び B 勤務に就くために
  間へ D 勤務を挟む必要が生じ、D への輸送（CD-arm）が発生する。
- これは単純な順序制約であり、サイクル回数や公平性は規定しない。
  D 利用は要員プールが逼迫したときに顕在化する（プールが潤沢なら D 利用は少ない）。
- §15 の together 同乗者（常駐に不要なカテゴリ）も、サイト滞在者である以上は
  本ローテーションの対象になる。
...
19. Vehicle Scheduling Constraints
...
21. Solver / Reference Implementation
...
参考実装: route_opt/（CP-SAT）。定式化は model.md を参照。
長 horizon（およそ3週間超）は単一モデルでは feasible 解の発見が困難なため、
ローリングホライズン分解（route_opt/rolling.py）で解く:
  - lookahead ウィンドウ（例 6日）で解き、commit（例 5日）までを確定して次へ進む。
  - ウィンドウ終端の状態（位置・到着時刻・D の残り必要滞在・直前勤務種別）を
    次ウィンドウの初期状態として引き継ぎ、§17 ローテーションを境界越しに維持する。
  - overlap = lookahead − commit は（最小の）滞在 max − outbound 所要 以下に取る。
規模・性能の詳細は BENCH.md を参照。

⸻

22. Open Items（[TBD] 一覧）
...
確定済（v0.3 で追記）:
- 必須ローテーション（乗客単位・B/D 交互順）が D 需要の駆動力（§17）
- together 同乗者はサイト滞在者になり stay・ローテーションの対象（§15）
- 滞在 max は期限が計画期間内のときのみ帰還を強制（§13）
- 参考実装 route_opt/ が CP-SAT＋ローリングホライズン分解で全 30 日を求解可能（§21, BENCH.md）

⸻

実装状況: spec の構造は確定し、参考実装（route_opt/）で B-arm/CD-arm・ローテーション・
ローリングホライズン・集約出力（CSV/Gantt）・GUI（apps/instance_editor.py）まで実装済み。
詳細は model.md（定式化）, BENCH.md（規模検証）を参照。
