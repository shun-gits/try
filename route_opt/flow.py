...
ダイヤグリッド上の「互換クラス内匿名フロー」定式化。ワーカーをコモディティ=(B サイト,
カテゴリ, 体重) 単位の整数フローとして扱い、個体識別は求解後の経路分解（decode）で復元する。
連続時間の自由変数・ワーカー対称性・弱い下界を排し、長 horizon を単発で扱える
（設計根拠と検証は BENCH.md 7章）。
...
class FlowModel:
    ...
    def _build_ghosts(self, transit_pids: dict[str, tuple[tuple[str, str], int]]) -> None:
        ...
        self.ghost_atD: list[tuple[int, int, int, str]] = []
    ...
    def _build(self):
        ...
        # D 同時在室上限（任意。ゴースト便＝初期 A->C の在室分も加算）。
        # per-weight（dict）なら weight 別に独立上限、int なら従来どおり総数上限。
        if self.d_occ_max is not None:
            per_w = isinstance(self.d_occ_max, dict)
            for g in range(H + 1):
                atD_by_w: dict[str, list] = {}
                for (tau, w, n), (t_ret, home) in ret_of.items():
                    arrD = tau + self.to_d
                    ...
                    if arrD <= g < leaveD:
                        for k in comm:
                            if k[2] == w and (k, tau, n) in bo:
                                atD_by_w.setdefault(w, []).append(bo[k, tau, n])
                for arr_g, leave_g, cnt, w in self.ghost_atD:
                    if arr_g <= g < leave_g:
                        atD_by_w.setdefault(w, []).append(cnt)
                if per_w:
                    for w, atD in atD_by_w.items():
                        cap = self.d_occ_max.get(w)
                        if cap is not None and atD:
                            m.Add(sum(atD) <= cap)
                else:
                    atD = [t for ts_ in atD_by_w.values() for t in ts_]
                    if atD:
