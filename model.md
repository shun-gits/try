...
B-arm(k) の往路便・復路便 j、together[k] の各カテゴリ群 G について（入域・退域に対称適用）:
(R1in)  hasCat_in[c,k,j]=1  ⇔ Σ_{p,m: cat(p)=c} inB[p,m,k,j] ≥ 1
(R2in)  G 内の全カテゴリで hasCat_in 一致（同一往路便で全員存在 or 全員不在）。
(R1out) hasCat_out[c,k,j]=1 ⇔ Σ_{p,m: cat(p)=c} outB[p,m,k,j] ≥ 1
(R2out) G 内の全カテゴリで hasCat_out 一致（同一復路便で全員存在 or 全員不在）。
※入域だけでなく退域も揃えることで、群はペアで島に入りペアで島を出る。
